        ...
        if args.dataset_name is not None:
            ...
            for path in image_paths:
                ...
                if caption_path.exists():
                    sidecar_prompts.append(caption_path.read_text(encoding="utf-8").strip())
                else:
                    sidecar_prompts.append("")
            if any(sidecar_prompts):
                self.custom_instance_prompts = []
                for caption in sidecar_prompts:
                    self.custom_instance_prompts.extend(itertools.repeat(caption or instance_prompt, repeats))
            else:
                self.custom_instance_prompts = None
