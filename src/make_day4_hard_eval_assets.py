...
def write_analysis_note(path: Path, summary_rows: list[dict]) -> None:
    lines = [
        "# Day 4 Hard Prompt Analysis Note",
        "",
        "## Purpose",
        "",
        "The hard-prompt subset checks whether the methods remain visually comparable on multi-object scenes, relations, text rendering, and dense composition.",
        "",
        "## Hard Subset",
        "",
        "Prompt indices: 18, 19, 21, 22, 23, 26, 28, 30.",
        "",
        "Compared methods: Fixed-20, Fixed-28, Rule-PCAS, Balanced-PCAS, and DeepSeek-Balanced.",
        "",
    ...
    lines.extend(
        [
            "",
            "Automatic scores remain close across methods. Visual differences mostly appear as local texture, edge, lighting, or small-object changes rather than stable semantic or composition improvements.",
            "",
            "## Interpretation",
            "",
            "PCAS should be interpreted as an efficiency-quality tradeoff strategy. The current evidence supports preserving automatic alignment and visual appearance under lower compute, not a proven improvement in visual quality.",
            "",
            "## Related Artifacts",
            "",
            "- Hard prompt grid: `results/figures/day4_hard_prompt_qualitative_grid.png`",
            "- Hard subset CLIPScore table: `results/day4_hard_prompt_clipscore_summary.csv`",
            "- Difference heatmap vs Fixed-20: `results/figures/day4_hard_prompt_difference_heatmap_vs_fixed20.png`",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    write_analysis_note(Path("results/day4_hard_prompt_analysis_note.md"), summary_rows)
